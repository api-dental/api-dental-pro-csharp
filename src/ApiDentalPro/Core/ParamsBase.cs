...
public abstract record class ParamsBase
{
    ...
    static ParamsBase()
    {
        ...
        var customHeadersEnv = Environment.GetEnvironmentVariable("API_DENTAL_PRO_CUSTOM_HEADERS");
    ...
    internal static void AddDefaultHeaders(HttpRequestMessage request, ClientOptions options)
    {
        ...
        {
            request.Headers.Add("X-Token-API", options.ApiKey);
        }
        request.Headers.Add(
            "X-Stainless-Timeout",
    ...
    static string GetUserAgent() => $"{typeof(ApiDentalProClient).Name}/C# {GetPackageVersion()}";
